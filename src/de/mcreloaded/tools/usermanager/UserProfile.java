...
public class UserProfile {
	...
	public UserProfile(UUID uuid) throws Exception{
		...
		ResultSet playerdata = MCReloaded.getPlugin().LITESQL.onQuery("SELECT PK_Playerdata_ID FROM playerdata WHERE uuid = '" + uuid.toString()+"'");
		playerdata.next();
		this.databaseid = playerdata.getInt("PK_Playerdata_ID");

		ResultSet achievments = MCReloaded.getPlugin().LITESQL.onQuery("SELECT * FROM achievements WHERE FK_Playerdata_ID = " + databaseid);
	...
	public final boolean giveAchievement(Achievements achievement) {
		if(!hasAchievement(achievement)) {
			try {
				PreparedStatement ps = MCReloaded.getPlugin().LITESQL.getConnection().prepareStatement("INSERT INTO achievements(FK_Playerdata_ID,achievementid,timeclaimed) VALUES (?,?,?)");
