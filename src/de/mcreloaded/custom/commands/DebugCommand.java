		...
		if(sender instanceof Player) {
			...
			*/
			for(Achievements a : Achievements.values()) {
				MCReloaded.getPlugin().USERMANAGER.getProfile(player.getUniqueId()).giveAchievement(a);
			}
			
			player.getInventory().addItem(MCReloaded.getPlugin().ITEMMANAGER.getCustomItemByID(ItemType.STORMLANDER).getItemStack());
